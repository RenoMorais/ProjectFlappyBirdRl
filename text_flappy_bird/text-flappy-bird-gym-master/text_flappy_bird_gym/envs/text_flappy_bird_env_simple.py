...
class TextFlappyBirdEnvSimple(gym.Env):
    ...
    def __init__(self, height=15, width=20, pipe_gap=4):
        ...
        self.observation_space = gym.spaces.Tuple(
            (
                ...
                gym.spaces.Discrete(y_dist_max - y_dist_min, start=y_dist_min),
            )
        )
        self._game = None
    ...
    def _get_observation(self):
        ...
        """
        closest_upcoming_pipe = min(
            [i for i, p in enumerate([pipe["x"] - self._game.player_x for pipe in self._game.upper_pipes]) if p >= 0]
        )
